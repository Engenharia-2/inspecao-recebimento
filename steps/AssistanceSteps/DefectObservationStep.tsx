...
const DefectObservationStep: React.FC = () => {
  // Filtra os campos para mostrar apenas os da etapa de 'assistance'
  const allCustomFields = useAppStore((state) => state.customFields);
  const customFields = useMemo(() =>
    (allCustomFields || []).filter(field => field.stage === 'assistance_defect'),
    [allCustomFields]
  );
  ...
  const handleAddField = (title: string) => {
    ...
    addCustomField(title, 'assistance_defect');
    setIsModalVisible(false);
  };

  return (
    <SafeAreaView style={styles.container}>
      <CustomTitle title='Observação de Defeitos'/>
        <Text style={styles.label}>
          Adicione campos para descrever o defeito, causa, solução ou outras observações relevantes.
        </Text>
        <CustomButton
...
export default React.memo(DefectObservationStep);